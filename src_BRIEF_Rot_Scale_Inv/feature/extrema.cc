...
#define ANGLE_STEP 0.017453 //(2.0*M_PI/NUM_ANGLES)
...
int calcAngle(Coor coor, const Mat32f& img)// int patchCalc, int **patch)
{
 	int size = SIZE;
    int half = size/2;
    ...
    int x = coor.x;
	int y = coor.y;	
	if((x <= half) || (y <= half) || (x >= (img.width() - half)) || (y >= (img.height() - half)))
		return -1;
	...
	// Get angle in radian from -PI to PI	
...
*/
	...
	{
	int npyramid = dog.noctave;// nscale = dog.nscale;
	#pragma omp for schedule(static,nscale)
	REP(i, npyramid)
		REPL(j, 1, nscale - 2) {
			auto v = get_local_raw_extrema(i, j);
			for (auto& c : v) 
...
/*				sp.angle = calcAngle(sp.coor, img, patchCalc, patch);				
				if(patchCalc == 1)
					patchCalc = 0;
				
				sp.angle = calcAngle(sp.coor, img, patchCalc, patch);
*/
//				sp.angle = calcAngle(sp.coor, img);
...
{
	static int curvatureThresh = sqr(EDGE_RATIO + 1)/EDGE_RATIO;

	float dxx, dxy, dyy;
	int x = coor.x, y = coor.y;
	float val = img.at(y, x);

	dxx = img.at(y, x + 1) + img.at(y, x - 1) - 2*val;
	dyy = img.at(y + 1, x) + img.at(y - 1, x) - 2*val;
