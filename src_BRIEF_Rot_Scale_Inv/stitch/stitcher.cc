...
bool Stitcher::match_image(
    const FeatureMatcher& pwmatcher, int i, int j) 
{

  auto match = pwmatcher.match();
...
#pragma omp parallel for schedule(dynamic)
  REP(k, (int)tasks.size()) 
  {
	...
	FeatureMatcher pwmatcher(feats[i],feats[j]);
...
#pragma omp parallel for schedule(dynamic)
  ...
  {
    int next = (i + 1) % n;
    FeatureMatcher pwmatcher(feats[i],feats[next]);
