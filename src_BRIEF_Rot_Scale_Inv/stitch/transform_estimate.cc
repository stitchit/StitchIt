...
bool TransformEstimation::get_transform(MatchInfo* info) 
{
    ...
    int step = 1000;
	int parts = RANSAC_ITERATIONS/step;

	//int partRansacIterations = RANSAC_ITERATIONS;
	for(int x = 0; x < parts; x++)
	{
		if(maxinlierscnt > ESTIMATE_MIN_NR_MATCH)
			break;
...
//	printf("Number of inliers = %d\n",inliers.size());
	...
	{
		//printf("Inliers size less than reqd. Returning false\n");
		return false;
	}
  ...
  // TODO shouldn't count undistinctive keypoints as keypoints. They should get filtered out earlier
	...
	//Homography inv = homoM.pseudo_inverse();

	/*for(int x = 0; x < 3; x++)
	{
		for(int y = 0; y < 3; y++)
			printf("%f\t", inv.data[x*3+y]);
		printf("\n");
	}*/
  ...
  // TODO guess if two images are identical
	...
	{
		printf("Confidence = %f, less than reqd %f\n",info->confidence,INLIER_IN_POINTS_RATIO);
		return false;
	}
