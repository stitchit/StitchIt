...
void gaussCuda(const float *img, float *kernel, float *retPtr, int h, int w, int kw);
...
class GaussianBlur {
	...
	public:
		...
		Mat32f blurCu(const Mat32f& img) const 
		{
			...
			float *kernel = gcache.kernel - kw/2;

            float *retPtr = ret.ptr(0,0);
            const float *imgPtr = img.ptr(0,0);
            gaussCuda(imgPtr, kernel, retPtr, h, w, kw);
		
            return ret;
        }

        Mat32f blurVec(const Mat32f& img) const 
        {
            m_assert(img.channels() == 1); 
            TotalTimer tm("gaussianblur");
...
//								out += img.at(rowStart,w-1) * kernel[k];
				...
				{
					#pragma omp parallel for
					for(int j=(limit); j < (limit+center); j++)
        ...
        Mat32f blurNaive(const Mat32f& img) const 
        {
            m_assert(img.channels() == 1); 
            TotalTimer tm("gaussianblur");
